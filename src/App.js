...
function App() {
  ...
  return (
    <div className="App">
      ...
      // pred.filter((e)=>e>1).map((e,key)=> {
      //   // if(e>=1){
        // return <div key={key}>{desis[key]}</div>
        <div key={pred}>{desis[pred]}</div>
        // }
              //  })
      }
      {picSrc && <img src={picSrc} ref={camraImg}  onClick={(e)=>handlepredict(camraImg)} id='snap' alt='snapshot'/>}
